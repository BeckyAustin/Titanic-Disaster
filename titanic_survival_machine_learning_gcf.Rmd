...
Fill in missing Embarked values in training dataset by assumming they got on in Southampton

```{r}
rf_train <- rf_train %>%
  dplyr::mutate(Embarked = dplyr::if_else(Embarked == "", "S", Embarked))
```
...
```{r}
set.seed(415)
rf_fit <- randomForest::randomForest(as.factor(Survived) ~ Pclass + Sex + Age + SibSp + Parch + Fare + Embarked,
                data = rf_train,
                importance = TRUE,
                ntree = 2000) %T>% 
...
submit <- rf_test %>%
  dplyr::mutate(Survived = prediction) %>% 
  dplyr::select(PassengerId, Survived) %T>% 
  readr::write_csv(here::here("myfirstforest.csv"))
```
